...
void Optimise::Solve_LbeamStress_With_NewtonRaphson (double reduced_move_limit)
{
    ...
    */
      ...
      lambda_g = reduced_move_limit;

      double fraction_area = 0.25; // delta_area will be 50 percent of maximum area
      double target_area = boundary_area;
      for(int i = 0; i<bpointsize; i++){
        target_area += boundaryPoints[i].length*fraction_area*(-lambda_g);
      }
      ...
      // compute upper and lower bounds for each boundary point
      ...
      for (int i = 0; i < bpointsize; i++)
      {
        ...
        // This computes the domain distance from the L-domain
        // Hardcoding here!
        domdist = std::min({ std::abs( curpt[0] - 0.0),std::abs( curpt[0] - length_x),std::abs( curpt[1] - 0.0),std::abs( curpt[1] - length_y) });

        // domain distance corresponding to the square hole
        double domdist1 = std::min({ curpt[0] - 0.4*length_x , -curpt[0] + length_x , curpt[1] - 0.4*length_y , -curpt[1] + length_y  });

        domdist = std::min( {-1.0*domdist1 , domdist } );

        upper_bound_vector[i] = std::min( {domdist , reduced_move_limit } );

        lower_bound_vector[i] = -reduced_move_limit;
      ...
      double default_area = boundary_area;
      for(int i = 0; i<bpointsize; i++){
        double z_i = std::min( upper_bound_vector[i], lambda_g*boundaryPoints[i].sensitivities[1] + lambda_0*boundaryPoints[i].sensitivities[0] ) ;
        z_i = std::max( lower_bound_vector[i],z_i ) ;

        default_area += boundaryPoints[i].length*z_i;
      }

      // Newton-Raphson iterations start here
      for (int iter_NR = 0; iter_NR < 250; iter_NR ++)
      {
        lambda_cur = lambda_0 + 0*delta_lambda;
        // compute new area
        double new_area0 = boundary_area;
        for(int i = 0; i<bpointsize; i++){
          double z_i = std::min( upper_bound_vector[i], lambda_g*boundaryPoints[i].sensitivities[1] + lambda_cur*boundaryPoints[i].sensitivities[0] ) ;
          z_i = std::max( lower_bound_vector[i],z_i ) ;

          new_area0 += boundaryPoints[i].length*z_i;
        }
        ...
        for(int i = 0; i<bpointsize; i++){
          double z_i = std::min( upper_bound_vector[i], lambda_g*boundaryPoints[i].sensitivities[1] + lambda_cur*boundaryPoints[i].sensitivities[0] ) ;
          z_i = std::max( lower_bound_vector[i],z_i ) ;
          new_area2 += boundaryPoints[i].length*z_i;
        }
        ...
        for(int i = 0; i<bpointsize; i++){
          double z_i = std::min( upper_bound_vector[i], lambda_g*boundaryPoints[i].sensitivities[1] + lambda_cur*boundaryPoints[i].sensitivities[0] ) ;
          z_i = std::max( lower_bound_vector[i],z_i ) ;
          new_area1 += boundaryPoints[i].length*z_i;
        }
        ...
        if(std::abs(new_area0 - target_area)/mesh_area < 1.0E-3) break;
      }
      ...
      for (int i = 0; i < bpointsize; i++)
      {
        double z_i = std::min( lambda_f*boundaryPoints[i].sensitivities[0] + lambda_g*boundaryPoints[i].sensitivities[1],  upper_bound_vector[i] );
          z_i = std::max( lower_bound_vector[i],z_i ) ;

        boundaryPoints[i].velocity = -1.0*z_i;

      }

      // cfl condition: if abs(velocity) > moveLimit then scale velocities appropriately
      double absvel = 0.0;
      for (int i = 0; i < bpointsize; i++) absvel = std::max(absvel , std::abs(boundaryPoints[i].velocity));
